...
export const getAppointmentsByCustomer = async (customerId: number): Promise<AppointmentWithDetails[]> => {
  try {
    const results = await db.select()
      ...
      .where(eq(appointmentsTable.customer_id, customerId))
      .execute();
    ...
    return results.map(result => ({
      ...
      appointment_date: new Date(result.appointments.appointment_date), // Convert string to Date
    ...
    console.error('Failed to get appointments by customer:', error);
    throw error;
  }
};
